...
% Get the maximum value in order to normalize each score
T_Max = max(max(BiosecurIDparameters(:,:,1)));
Npu_Max = max(max(BiosecurIDparameters(:,:,2)));
Tpd_Max = max(max(BiosecurIDparameters(:,:,3)));
Ppd_Max = max(max(BiosecurIDparameters(:,:,4)));
Max_values = [T_Max,Npu_Max,Tpd_Max,Ppd_Max];
